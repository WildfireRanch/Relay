...
# Directory: services/context_engine.py
...
class ContextEngine:
    """
    Gather per-user runtime context from code, docs, logs, and knowledge base.
    Supports multi-turn context keyed by user_id.
    """
    ...
    def read_source_files(self,
                          ...
                          exts: Optional[List[str]] = None) -> str:
        """
        ...
        for root in roots:
            ...
            if not path.exists():
                continue
            for f in path.rglob("*"):
                if f.suffix in exts and f.is_file() and not any(ex in str(f) for ex in excluded):
                    try:
    ...
    def build_context(self, query: str) -> str:
        ...
        if self.needs_code_context(query):
            ...
            # Append KB summaries for user
            kb_summary = kb.get_recent_summaries(self.user_id)
            context = f"{code}\n\n{docs}\n\nLogs:\n{logs}\n\nKB Summary:\n{kb_summary}"
        else:
